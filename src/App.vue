...
<template>
  <NotificationGroup group="foo">
    <div class="fixed inset-0 flex items-start justify-end p-6 px-4 py-6 pointer-events-none absolute z-100">
      <div class="w-full max-w-sm absolute z-100">
        <Notification v-slot="{ notifications }" enter="transform ease-out duration-300 transition"
          ...
          <div class="flex w-full max-w-sm mx-auto mt-4 overflow-hidden bg-dark rounded-lg shadow-md absolute z-100"
            ...
            <div class="px-4 py-2 -mx-3">
              <div class="mx-3">
                ...
                <p class="text-sm text-gray-100">{{ notification.text }}</p>
              </div>
            </div>
          </div>
        </Notification>
    ...
    <div class="fixed inset-0 flex items-start justify-end p-6 px-4 py-6 pointer-events-none absolute z-100">
      <div class="w-full max-w-sm">
        <Notification v-slot="{ notifications }" enter="transform ease-out duration-300 transition"
          ...
          enter-to="translate-y-0 opacity-100 sm:translate-x-0" leave="transition ease-in duration-500"
          leave-from="opacity-100" leave-to="opacity-0" move="transition duration-500" move-delay="delay-300">
          <div class="flex w-full max-w-sm mx-auto mt-4 overflow-hidden bg-dark rounded-lg shadow-md"
            v-for="notification in notifications" :key="notification.id">
...
<style>
@font-face { font-family: Katibeh; src: url('./assets/fonts/Katibeh-Regular.ttf'); } 
@font-face { font-family: BalooBhaijaan; src: url('./assets/fonts/BalooBhaijaan2-Bold.ttf'); } 
#app {
  width: 100%;
  height: 100%;
  position: absolute;
  display: flex;
  font-family: Roboto, Helvetica, BalooBhaijaan, sans-serif;
}
